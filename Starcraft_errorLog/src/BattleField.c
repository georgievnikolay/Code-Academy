...
void generateTerranFleet(BattleField *battleField, const char *terranFleetStr) {
  ...
  CHECK_ALLOCATION_ERROR(terranFleet); //<<---- 
  ...
  for (i = 0; i < strlen(terranFleetStr); i++) {
    switch(terranFleetStr[i]){
      ...
      case VIKING:
        ...
        break;
      default: //<<---- 
        printf("%c is invalid input: expected 'b' or 'v' for the Terran\n", terranFleetStr[i]);
        exit(EINVAL);
    }
...
void generateProtossFleet(BattleField *battleField, const char *protossFleetStr) {
  ...
  CHECK_ALLOCATION_ERROR(protossFleet); //<<---
  ...
  for (i = 0; i < strlen(protossFleetStr); i++) {
    switch(protossFleetStr[i]){
      ...
      case PHOENIX:
        ...
        break;
      default: //<<---- 
        printf("%c is invalid input: expected 'c' or 'p' for the Protoss\n", protossFleetStr[i]);
        exit(EINVAL);
    }
